...
from argus.llm import LLMRouter, ModelConfig, SpendConfig, SpendTracker
...
async def test_cost_hook_aborts_when_cap_exceeded():
    ...
    handlers = {
        state: counting_handler for state in TaskState if state != TaskState.ABORT
    }
...
async def test_real_anthropic_call_completes():
    ...
    router = LLMRouter(config=config, tracker=tracker)
    ctx = RunContext(
        task_id="integration-llm-01", task_input={"goal": "say hello in one word"}
    )
