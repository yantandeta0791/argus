...
async def test_input_validation_rejects(mock_gateway):
    ...
    manifest = ToolManifest(name="read_file", input_schema=_ReadInput, output_schema=_ReadOutput)
...
async def test_retry_exponential_backoff(mock_gateway):
    ...
    manifest = ToolManifest(
        ...
        idempotent=True, max_attempts=3, backoff_base=0.01,  # fast backoff for tests
    )
    call_count = {"n": 0}
...
async def test_circuit_breaker_opens(mock_gateway):
    ...
    from argus.engine.tools import ToolManifest, ToolRunner
    manifest = ToolManifest(
        name="flaky_write", input_schema=_ReadInput, output_schema=_ReadOutput,
        idempotent=True, max_attempts=1, failure_threshold=3, recovery_timeout=60,
    )
...
async def test_non_idempotent_no_retry_on_ambiguous(mock_gateway):
    ...
    from argus.engine.tools import ToolManifest, ToolRunner
    manifest = ToolManifest(
        name="write_file", input_schema=_ReadInput, output_schema=_ReadOutput,
        idempotent=False, max_attempts=3, backoff_base=0.01,
    )
    call_count = {"n": 0}
