...
def test_builtin_tier_rejected_external(tmp_path):
    ...
    m = SkillManifest(
        name="fake-builtin", version="1.0.0", description="test",
        trust_tier="builtin", permissions=[], content_hash="sha256:" + "a" * 64,
    )
...
def test_blast_radius_enum():
    ...
    m = SkillManifest(
        ...
        trust_tier="verified", permissions=[], content_hash="sha256:" + "a" * 64,
        blast_radius="local",
    )
...
def test_content_hash_prefix_validation():
    ...
    with pytest.raises(ValidationError, match="content_hash"):
        SkillManifest(
            name="test", version="1.0.0", description="test",
            trust_tier="verified", permissions=[], content_hash="bad_hash",
        )
